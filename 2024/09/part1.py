...
def get_leftmost_space_index():
    for leftmost_space_index in range(len(spans)):
        if (not spans[leftmost_space_index].is_file) and (spans[leftmost_space_index].length > 0):
            return leftmost_space_index

def get_rightmost_file_index():
    for rightmost_file_index in range(len(spans) - 1, 0, -1):
        if spans[rightmost_file_index].is_file and (spans[rightmost_file_index].length > 0):
...
while True:
    ...
    if leftmost_space_index > rightmost_file_index:
        break
    length_to_swap = min(spans[leftmost_space_index].length, spans[rightmost_file_index].length)
